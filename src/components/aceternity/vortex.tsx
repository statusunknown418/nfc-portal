...
export const Vortex = (props: VortexProps) => {
  ...
  const updateParticle = (i: number, ctx: CanvasRenderingContext2D) => {
    ...
    x = particleProps[i];
    y = particleProps[i2];
    n = noise3D(Number(x) * xOff, Number(y) * yOff, tick * zOff) * noiseSteps * TAU;
    vx = lerp(Number(particleProps[i3]), Math.cos(n), 0.5);
    vy = lerp(Number(particleProps[i4]), Math.sin(n), 0.5);
    life = particleProps[i5];
    ttl = particleProps[i6];
    speed = particleProps[i7];
    x2 = Number(x) + vx * Number(speed);
    y2 = Number(y) + vy * Number(speed);
    ...
    particleProps[i3] = vx;
    particleProps[i4] = vy;
    particleProps[i5] = Number(life);

    (checkBounds(Number(x), Number(y), canvas) ?? Number(life) > Number(ttl)) && initParticle(i);
  };
